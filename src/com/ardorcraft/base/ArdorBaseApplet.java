...
/**
 * An example base class for ardor3d/lwjgl applets. This is not meant to be a
 * "best-practices" applet, just a rough demo showing possibilities. As such,
 * there are likely bugs, etc. Please report these. :)
 */
public abstract class ArdorBaseApplet extends Applet implements Scene {
		...
		try {
			...
			_displayCanvas.addComponentListener(new ComponentAdapter() {
				...
				public void componentResized(final ComponentEvent e) {
					ArdorCraftTaskQueueManager.getManager(
							_glCanvas.getCanvasRenderer().getRenderContext())
							.update(new Callable<Void>() {
								@Override
								public Void call() throws Exception {
									final Camera cam = _glCanvas
		...
		_gameThread = new Thread() {
			...
			public void run() {
				...
				try {
					...
					final CanvasRelayer canvas = new AppletCanvasRelayer(
							_glCanvas);
					ardorCraft.init(_root, canvas, _logicalLayer,
							_physicalLayer, _mouseManager);
	...
	protected void initGL() throws LWJGLException {
		...
		_glCanvas = new LwjglDisplayCanvas(_displayCanvas, _settings,
				canvasRenderer);
		_glCanvas.init();
	...
	protected void initInput() {
		...
		_logicalLayer.registerTrigger(new InputTrigger(
				new KeyReleasedCondition(Key.F5), new TriggerAction() {
					@Override
					public void perform(
							final com.ardor3d.framework.Canvas source,
							final TwoInputStates inputState, final double tpf) {
						try {
							_glCanvas.setFullScreen(!_glCanvas.isFullScreen());
							final Camera cam = _glCanvas.getCanvasRenderer()
									.getCamera();
							if (_glCanvas.isFullScreen()) {
								final DisplayMode mode = Display
										.getDisplayMode();
								cam.resize(mode.getWidth(), mode.getHeight());
								cam.setFrustumPerspective(
										cam.getFovY(),
										mode.getWidth()
												/ (float) mode.getHeight(),
										cam.getFrustumNear(),
										cam.getFrustumFar());
								ardorCraft.resize(mode.getWidth(),
										mode.getHeight());
							} else {
								cam.resize(getWidth(), getHeight());
								cam.setFrustumPerspective(cam.getFovY(),
