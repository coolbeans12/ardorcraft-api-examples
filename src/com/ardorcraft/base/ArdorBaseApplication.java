...
import com.ardorcraft.util.queue.ArdorCraftTaskQueueManager;

public abstract class ArdorBaseApplication implements Runnable, Updater, Scene {
	private static final Logger logger = Logger
			.getLogger(ArdorBaseApplication.class.getName());
	...
	public ArdorBaseApplication(final ArdorCraftGame ardorCraft) {
		...
		// Ask for properties
		final PropertiesGameSettings prefs = getAttributes(new PropertiesGameSettings(
				"ardorSettings.properties", null));

		// Convert to DisplayProperties (XXX: maybe merge these classes?)
		final DisplaySettings settings = new DisplaySettings(prefs.getWidth(),
				prefs.getHeight(), prefs.getDepth(), prefs.getFrequency(),
				// alpha
				_minAlphaBits != -1 ? _minAlphaBits : prefs.getAlphaBits(),
				// depth
				_minDepthBits != -1 ? _minDepthBits : prefs.getDepthBits(),
				// stencil
				_minStencilBits != -1 ? _minStencilBits
		...
		_physicalLayer = new PhysicalLayer(new LwjglKeyboardWrapper(),
	...
	@MainThread
	public void init() {
		final ContextCapabilities caps = ContextManager.getCurrentContext()
				.getCapabilities();
		...
		logger.info("Shading Language Version: "
				+ caps.getShadingLanguageVersion());
		...
		try {
			final SimpleResourceLocator srl = new SimpleResourceLocator(
					ResourceLocatorTool.getClassPathResource(
							ArdorBaseApplication.class,
							"com/ardorcraft/resources"));
		...
		ardorCraft.init(_root, canvas, _logicalLayer, _physicalLayer,
				_mouseManager);
	...
	public void update(final ReadOnlyTimer timer) {
		...
		ArdorCraftTaskQueueManager
				.getManager(_canvas.getCanvasRenderer().getRenderContext())
	...
	public boolean renderUnto(final Renderer renderer) {
		...
		ArdorCraftTaskQueueManager
				.getManager(_canvas.getCanvasRenderer().getRenderContext())
				.getQueue(ArdorCraftTaskQueue.RENDER).execute(renderer);
		GameTaskQueueManager
				.getManager(_canvas.getCanvasRenderer().getRenderContext())
				.getQueue(ArdorCraftTaskQueue.RENDER).execute(renderer);
		...
		if (!_canvas.isClosing()) {
			...
			if (_doShot) {
				...
				ScreenExporter.exportCurrentScreen(_canvas.getCanvasRenderer()
						.getRenderer(), _screenShotExp);
				_doShot = false;
			}
			return true;
		} else {
	...
	protected void processPicks(final PrimitivePickResults pickResults) {
		...
		while (pickResults.getNumber() > 0
				&& pickResults.getPickData(i).getIntersectionRecord()
						.getNumberOfIntersections() == 0
	...
	protected static PropertiesGameSettings getAttributes(
			final PropertiesGameSettings settings) {
		// Always show the dialog in these examples.
		URL dialogImage = null;
		final String dflt = settings.getDefaultSettingsWidgetImage();
		if (dflt != null) {
			try {
				dialogImage = ResourceLocatorTool.getClassPathResource(
						ArdorBaseApplication.class, dflt);
			} catch (final Exception e) {
				logger.log(Level.SEVERE, "Resource lookup of '" + dflt
						+ "' failed.  Proceeding.");
			}
		}
		if (dialogImage == null) {
		...
		try {
			if (EventQueue.isDispatchThread()) {
				dialogRef.set(new PropertiesDialog(settings, dialogImageRef,
						mainThreadTasks));
			} else {
				EventQueue.invokeLater(new Runnable() {
					@Override
					public void run() {
						dialogRef.set(new PropertiesDialog(settings,
								dialogImageRef, mainThreadTasks));
					}
				});
			}
		} catch (final Exception e) {
			logger.logp(Level.SEVERE, ArdorBaseApplication.class.getClass()
					.toString(),
					"ArdorBaseApplication.getAttributes(settings)",
					"Exception", e);
			return null;
		}
	...
	protected void registerInputTriggers() {
		...
		_logicalLayer.registerTrigger(new InputTrigger(new KeyPressedCondition(
