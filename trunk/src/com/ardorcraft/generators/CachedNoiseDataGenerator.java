...
public class CachedNoiseDataGenerator extends LayerDataGenerator {
        ...
        for (int xs = 0; xs < wSize; xs++) {
            for (int ys = 0; ys < hSize; ys++) {
                for (int zs = 0; zs < wSize; zs++) {
                    ...
                    noiseCache1[xs][ys][zs] = ImprovedNoise.noise(x * 0.005, 0.5, z * 0.005);
                    noiseCache2[xs][ys][zs] = ImprovedNoise.noise(x * 0.02, y * 0.02, z * 0.02);
                    noiseCache3[xs][ys][zs] = ImprovedNoise.noise(x * 0.02, 1.0, z * 0.02);
                    noiseCache4[xs][ys][zs] = ImprovedNoise.noise(x * 0.05, 1.0, z * 0.05);
                }
            }
        }
    ...
    public boolean isCave(final int x, final int y, final int z, final WorldModifier blockScene) {
        if (y > 1) {
            ...
            if (y > testHeight) {
                return interpolatedNoise(noiseCache2, x, y, z) > Math.abs(y - testHeight) / 60.0;
            } else {
                return interpolatedNoise(noiseCache2, x, y, z) > Math.abs(y - testHeight) / 40.0 + 0.1;
            }
        }
        return false;
    }
