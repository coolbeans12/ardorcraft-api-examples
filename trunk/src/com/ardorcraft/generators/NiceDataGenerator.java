...
public class NiceDataGenerator implements DataGenerator {
	private final int waterHeight = 11;
	...
	private void generateColumn(final int x, final int z, final int height,
			final WorldModifier blockScene) {
		final double gen = 5;

		int localHeight = 0;

		localHeight = generateLayer(x, z, gen + 0, 0.4, 0, 0.1f * height, 17,
				0.75f, height, blockScene);
		localHeight = generateLayer(x, z, gen + 0.2, 1.5, localHeight, 0.08f
				* height + 2.0f * (localHeight - 10), 2, 0.8f, height,
				blockScene);
		localHeight = generateLayer(x, z, gen + 0.5, 2.0, localHeight, 0.06f
				* height + 0.3f * (localHeight - 5), 3, 0.6f, height,
				blockScene);
		...
		for (int y = localHeight; y < localHeight + val; y++) {
			...
			final double scaleY = (Math.abs(y - height / 5) + 10.0) / height
					* 3.5;
			final double scale = 0.05;
			final double noise4 = ImprovedNoise.noise(x * scale, y * scale
					* 2.5, z * scale);
			if (noise4 < scaleY) {
		...
		final int block = blockScene.getBlock(x, localHeight - 1, z,
				BlockType.Solid);
		if (localHeight > waterHeight && block == 3) {
			blockScene.setBlock(x, localHeight - 1, z, 4);

			if (noise2 < -0.4) {
				final double noiseTree = ImprovedNoise.noise(x * 0.2,
						localHeight * 0.2, z * 0.2);
				if (noiseTree > 0.4) {
	...
	private int generateLayer(final int x, final int z, final double noiseVal,
			final double noiseScale, final int startheight, float layerheight,
			final int type, final float adder, final int height,
			final WorldModifier blockScene) {
		layerheight = Math.max(0.0f, layerheight);

		double noise = ImprovedNoise.noise(x * 0.01 * noiseScale, noiseVal, z
				* 0.01 * noiseScale)
				+ adder;
		double noise2 = ImprovedNoise.noise(x * 0.05 * noiseScale, noiseVal, z
				* 0.05 * noiseScale)
				+ adder;
		...
		int localHeight = (int) (noise * layerheight + noise2 * layerheight
				* 0.35);
		localHeight = Math.max(0, localHeight);

		for (int y = startheight; y < startheight + localHeight; y++) {
			if (y <= 1) {
				blockScene.setBlock(x, y, z, 18);
			}
			final double scaleY = (Math.abs(y - height / 3) + 15.0) / height
					* 1.5;
			final double scale = 0.05;
			final double noise3 = ImprovedNoise.noise(x * scale, y * scale
					* 2.0, z * scale);
			if (noise3 < scaleY) {
				if (type == 2 && localHeight == 1 && y > waterHeight) {
